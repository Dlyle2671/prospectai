    ...
    <div>
      ...
      <div className="sa-g3">
        ...
        <div className="sa-stat">
          ...
          <div className="sub">Month {CM} of 12 {Array.from({length:12},(_,i)=>i<CM?'█':'░').join('')} where you should be</div>
        </div>
      </div>
  ...
  return(
    <div>
      <div style={{background:'rgba(99,102,241,.08)',border:'1px solid rgba(99,102,241,.2)',borderRadius:10,padding:'12px 16px',marginBottom:16,fontSize:13,color:'#a5b4fc',lineHeight:1.6}}>
        <strong>Deals</strong> ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ enter each closed deal by client. PS = one-time fee (not annualized). FO/MS = MRR x months remaining = ARR. Commission: PS=10% of fee | FO=7% of 1st month MRR | MS=1x MRR. All deals feed directly into the Dashboard and Commissions.
      </div>
      ...
      <div className="sa-card">
        <h2>Closed Deals {filterCat!=='All'?'ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ '+(filterCat==='PS'?'Professional Services':filterCat==='FO'?'FinOps':'Managed Services'):''} ({filtered.length})</h2>
        <table className="sa-tbl">
          <thead><tr>
          ...
          <tbody>
            {filtered.length===0&&(
              <tr><td colSpan={8} style={{textAlign:'center',color:'#fff',padding:24}}>
                No deals yet ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ click "+ Add Deal" to log a closed deal.
              </td></tr>
            )}
            {filtered.map(d=>{
    ...
    <div className="rdm-ov" onClick={e=>e.target.className==='rdm-ov'&&onClose()}>
      ...
      <div className="rdm-box">
        <div className="rdm-hd">
          <div>
            <div className="rdm-hd-title">ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {rep.name}</div>
            <div className="rdm-hd-sub">{rep.dept||rep.department||'Sales'} ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· {totalDeals} deal{totalDeals!==1?'s':''} won ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· {new Date().getFullYear()} Performance</div>
          </div>
          <button className="rdm-hd-close" onClick={onClose}>ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ</button>
        </div>
        <div className="rdm-body">
          <div className="rdm-kpi">
            ...
            <div className="rdm-kpi-card">
              ...
              <div className="rdm-kpi-val" style={{color:overallAttain>=CM/12?'#34d399':'#f87171'}}>{totalQuota>0?pct(overallAttain):'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ'}</div>
              <div className="rdm-kpi-sub">vs. {fmt(totalQuota)} annual quota</div>
            </div>
            <div className="rdm-kpi-card">
              <div className="rdm-kpi-lbl">Pace vs. Attainment</div>
              <div className="rdm-kpi-val" style={{color:overallAttain>=CM/12?'#34d399':'#f87171'}}>{overallAttain>=CM/12?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ On Track':'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ  Behind'}</div>
            ...
            {catData.map(c=>(
              <div className="rdm-qcard" key={c.id}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <span className={'rdm-badge '+(c.arr>=c.quota*(CM/12)?'ahead':'behind')}>{c.quota>0?pct(c.arr/c.quota):'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ'}</span>
                  ...
                  <tbody>
                    {c.deals.map(d=>(
                      <tr key={d.id}>
                        ...
                        <td style={{color:'#64748b',textAlign:'center'}}>{c.id==='PS'?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ':mrem(d.month)}</td>
          ...
          <div className="sa-card" key={c.id}>
            ...
            <div className="sa-g3">
              <div className="sa-stat">
                ...
                <div className="val" style={{color:'#34d399'}}>{fmt(closed)}</div>
                <div className="sub">of {fmt(quota)} quota ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {pct(p)} attained</div>
                <div className="sa-bar"><div className="sa-bar-fill" style={{width:p*100+'%',background:'#34d399'}}/></div>
    ...
    <div>
      <div className="sa-card">
        ...
        <div style={{fontSize:12,color:'#fff',marginBottom:16,fontStyle:'italic',lineHeight:1.7}}>
          <strong style={{color:'#818cf8'}}>Professional Services:</strong> One-time project fee. Fee is the value ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ not annualized. Commission = 10% of fee.<br/>
          ...
          {calcCat!=='PS'&&(
            <div><label className="sa-label">Month Closed</label>
              <select className="sa-select" value={calcMonth} onChange={e=>setCalcMonth(Number(e.target.value))}>
                {MN.map((m,i)=><option key={i} value={i+1}>{m} ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {13-(i+1)} months remaining</option>)}
              </select>
            </div>
          )}
        </div>
      ...
      {calcCat!=='PS'&&(
        <div className="sa-card">
              ...
              {MN.map((m,i)=>{
                ...
                return(
                  <tr key={i} style={isCur?{background:'rgba(99,102,241,.08)'}:{}}>
                    <td style={isCur?{color:'#818cf8',fontWeight:600}:{}}>{m}{isCur?' ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ':''}</td>
                    <td>{r}</td>
          ...
          <div style={{fontSize:11,color:'#a5b4fc',marginTop:8,fontStyle:'italic'}}>
            {calcCat==='FO'?'Note: Commission = 7% of 1st month MRR ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ same regardless of month closed.':'Note: Commission = 1x MRR flat ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ same regardless of month closed.'}
          </div>
        </div>
      )}
    </div>
        ...
        <div className="sa-card" style={{marginTop:16}}>
          <h2 style={{fontSize:14,fontWeight:700,color:'#f1f5f9',margin:'0 0 14px',display:'flex',alignItems:'center',gap:8}}>
            ...
            <span style={{fontSize:11,fontWeight:500,color:'#ffffff'}}>
              {filterRep!=='All'&&repById[filterRep]}{filterRep!=='All'&&filterMonth!=='All'&&' ÃÂ· '}{filterMonth!=='All'&&MN[Number(filterMonth)-1]}
            </span>
          ...
          return(
            <div key={r.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',background:'#0f172a',borderRadius:10,marginBottom:8,border:'1px solid rgba(255,255,255,.06)'}}>
              <div>
                ...
                <div style={{fontSize:12,color:'#fff',marginTop:3}}>{r.dept||r.department||'ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ'} | {myDeals.length} deals | Total Closed: {fmt(tot)} | Commission: {fmt(comm)}</div>
              </div>
