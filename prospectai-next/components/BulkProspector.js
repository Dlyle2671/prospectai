        ...
        try {
                  for (let di = 0; di < total; di++) {
                    ...
                    const resp = await fetch('/api/apollo', {
                              ...
                              contacts.forEach(c => {
                                            if (!seenIds.has(c.id)) { seenIds.add(c.id); allContacts.push(c); }
                              });
    ...
    {/* ── File Upload ─────────────────────────────────────── */}
        ...
        <div
          ...
          style={{
                          ...
                          background: isDragOver ? 'rgba(59,130,246,0.07)' : '#0f172a', transition: 'all .2s', marginBottom: 12,
          }}
        >
...
{domains.slice(0, 50).map((d, i) => (
                    <div key={i} style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:11, color:'#94a3b8', background:'#111827', border:'1px solid #1e293b', borderRadius:5, padding:'2px 8px' }}>
                          {d}
                                            <span onClick={() => removeDomain(i)} style={{ cursor:'pointer', color:'#475569', marginLeft:2 }}>x</span>
    </div>
              ))}
...
{/* Table header */}
            <div style={{ display:'grid', gridTemplateColumns:'36px 1fr 1fr 180px 140px 60px', gap:10, padding:'8px 14px', borderBottom:'1px solid #1e293b', fontSize:11, color:'#475569', fontWeight:600, textTransform:'uppercase', letterSpacing:'.05em', alignItems:'center' }}>
              <div />
                              <div>Name / Title</div>
...
{results.map((p, i) => (
                  <div key={i} style={{ display:'grid', gridTemplateColumns:'36px 1fr 1fr 180px 140px 60px', gap:10, padding:'10px 14px', borderBottom:'1px solid #1e293b', alignItems:'center' }}>
             {/* Avatar */}
