...
const DEFAULT = { titles: ['VP of Engineering','CTO','Head of Infrastructure','Cloud Architect','DevOps Manager'] };
...
function parseCsvText(text) {
    ...
    if (!domainCol) return lines.flatMap(l => { const d = cleanDomain(l.trim()); return d ? [d] : []; }).filter(Boolean);
...
async function parseExcelFile(file) {
    ...
    const domainCol = ['domain','website','url','company_domain','company domain','site','homepage','web'].find(k => header.includes(k));
    if (!domainCol) return rows.flatMap(r => { const d = cleanDomain(String(r[0]||'')); return d ? [d] : []; }).filter(Boolean);
    const idx = header.indexOf(domainCol);
...
export default function BulkProspector() {
  ...
  function toggleTitle(val) {
        setFilters(prev => {
                const arr = [...prev.titles];
                const i = arr.indexOf(val);
                if (i >= 0) arr.splice(i, 1); else arr.push(val);
                return { ...prev, titles: arr };
        });
  }
  ...
  async function handleFile(file) {
        ...
        try {
                ...
                if (!parsed.length) { setFileError('No valid domains found. Make sure your file has a column named "domain", "website", or "url".'); return; }
                const unique = [...new Set(parsed)];
                setDomains(unique);
                paiSave('bulk_domains', unique);
        } catch (e) { setFileError('Failed to parse file: ' + e.message); }
  }

  function onFileInput(e) { handleFile(e.target.files[0]); e.target.value = ''; }
    function onDrop(e) { e.preventDefault(); setIsDragOver(false); handleFile(e.dataTransfer.files[0]); }
    function removeDomain(i) { const next = domains.filter((_, idx) => idx !== i); setDomains(next); paiSave('bulk_domains', next); }
    function clearDomains() { setDomains([]); setFileName(''); paiSave('bulk_domains', []); }

  async function runSearch() {
        if (!filters.titles.length) { alert('Select at least one job title.'); return; }
        if (!domains.length) { alert('Upload a CSV or Excel file with company domains first.'); return; }
        setStage(1); setResults([]); setProgress(0); setHsSent({});
        const allContacts = []; const seenIds = new Set(); const total = domains.length;
        try {
                for (let di = 0; di < total; di++) {
                          ...
                          const resp = await fetch('/api/apollo', {
                                      ...
                                      body: JSON.stringify({ per_page: 25, page: 1, person_titles: filters.titles, organization_domains: [domain] }),
                          });
                          const data = await resp.json();
                          const contacts = Array.isArray(data) ? data : (data.people || data.contacts || []);
                          contacts.forEach(c => { if (!seenIds.has(c.id)) { seenIds.add(c.id); allContacts.push(c); } });
                          await new Promise(r => setTimeout(r, 150));
                }
                setResults(allContacts);
                paiSave('bulk_results', allContacts);
                paiSave('bulk_filters', { titles: filters.titles });
                setStage(2); setProgress(100); setProgressMsg('');
  ...
  async function pushHS(i) {
        ...
        try {
                await fetch('/api/hubspot', { method: 'POST', headers: { 'Content-Type': 'application/json' },
                                                     body: JSON.stringify({ email: p.email, firstname: p.first_name, lastname: p.last_name, jobtitle: p.title, company: p.company_name, phone: p.personal_phone, linkedin: p.linkedin_url }) });
                setHsSent(prev => ({ ...prev, [i]: true }));
        } catch (err) { alert('HubSpot error: ' + err.message); }
  }

  function exportCSV() {
        const rows = [['Name','Title','Company','Domain','Email','Phone','LinkedIn','Location','Score']];
        results.forEach(p => rows.push([
                ((p.first_name||'') + ' ' + (p.last_name||'')).trim(), p.title||'', p.company_name||'', p.company_domain||'',
                p.email||'', p.personal_phone||'', p.linkedin_url||'',
                [p.city,p.state,p.country].filter(Boolean).join(', '), p.score||'',
              ]));
  ...
  return (
      ...
      <div style={boxStyle}>
        ...
        <div
          ...
          style={{ border:'2px dashed '+(isDragOver?'#3b82f6':'#334155'), borderRadius:10, padding:'28px 20px', textAlign:'center', cursor:'pointer', background:isDragOver?'rgba(59,130,246,0.07)':'#0f172a', transition:'all .2s', marginBottom:12 }}
        >
          <div style={{ fontSize:28, marginBottom:8 }}>📂</div>
          <div style={{ fontSize:13, color:'#94a3b8', marginBottom:4 }}>
{fileName ? ('Loaded: ' + fileName) : 'Drag & drop your file here, or click to browse'}
</div>
          <div style={{ fontSize:11, color:'#475569' }}>CSV or Excel (.xlsx) — max 500 domains</div>
            <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls,.xlsm" style={{ display:'none' }} onChange={onFileInput} />
  </div>
{fileError && (
...
{domains.slice(0,50).map((d,i) => (
                  <div key={i} style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:11, color:'#94a3b8', background:'#111827', border:'1px solid #1e293b', borderRadius:5, padding:'2px 8px' }}>
                         {d}<span onClick={() => removeDomain(i)} style={{ cursor:'pointer', color:'#475569', marginLeft:2 }}>x</span>
  </div>
              ))}
{domains.length > 50 && <div style={{ fontSize:11, color:'#475569', padding:'2px 8px' }}>and {domains.length - 50} more</div>}
      ...
      <div style={boxStyle}>
        ...
        {TITLE_OPTIONS.map(t => (
                      <button key={t} className={'pill ' + (filters.titles.includes(t) ? 'active' : '')} onClick={() => toggleTitle(t)}>{t}</button>
          ))}
</div>
  </div>

      <div style={boxStyle}>
          <div style={labelStyle}>Step 3 — Run Search</div>
        <button
...
{stage === 2 && results.length > 0 && (
          ...
          <div style={{ background:'#0f172a', borderTop:'1px solid #334155' }}>
            <div style={{ display:'grid', gridTemplateColumns:'36px 1fr 1fr 170px 130px 70px', gap:10, padding:'8px 14px', borderBottom:'1px solid #1e293b', fontSize:11, color:'#475569', fontWeight:600, textTransform:'uppercase', letterSpacing:'.05em', alignItems:'center' }}>
              <div /><div>Name / Title</div><div>Company / Domain</div><div>Email</div><div>Phone</div><div>Action</div>
  </div>
{results.map((p, i) => (
                <div key={i} style={{ display:'grid', gridTemplateColumns:'36px 1fr 1fr 170px 130px 70px', gap:10, padding:'10px 14px', borderBottom:'1px solid #1e293b', alignItems:'center' }}>
             ...
             {((p.first_name||'?')[0]||'?').toUpperCase()}
                ...
                <div>
                    <div style={{ fontSize:12, color:'#c8d4e8' }}>{p.company_name||'—'}</div>
                  <div style={{ fontSize:11, color:'#475569' }}>{p.company_domain||''}</div>
  </div>
                <div style={{ fontSize:11, color:'#64748b', wordBreak:'break-all' }}>{p.email||'—'}</div>
                <div style={{ fontSize:11 }}>
{p.personal_phone ? <a href={'tel:'+p.personal_phone} style={{ color:'#38bdf8', textDecoration:'none', fontWeight:500 }}>{p.personal_phone}</a> : <span style={{ color:'#334155' }}>—</span>}
  </div>
                <button
                  style={{ padding:'5px 10px', borderRadius:6, border:'none', background:hsSent[i]?'#14532d':'#ff7a59', color:hsSent[i]?'#4ade80':'#fff', fontSize:11, cursor:hsSent[i]?'default':'pointer', whiteSpace:'nowrap' }}
