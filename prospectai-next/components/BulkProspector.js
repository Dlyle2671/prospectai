...
function parseCsvText(text) {
        ...
        if (!domainCol) {
                  return lines.flatMap(l => { const d = cleanDomain(l.trim()); return d ? [d] : []; }).filter(Boolean);
        }
...
export default function BulkProspector() {
  ...
  async function handleFile(file) {
            ...
            try {
                        ...
                        setDomains(unique); paiSave('bulk_domains', unique);
            ...
            try {
                        for (let di = 0; di < total; di++) {
                                      ...
                                      const resp = await fetch('/api/apollo', {
                                                      ...
                                                      body: JSON.stringify({ per_page: Math.min(filters.limit, 25), page: 1, person_titles: filters.titles, organization_num_employees_ranges: filters.sizes, organization_domains: [domain] }),
                                      });
                        ...
                        setResults(allContacts); paiSave('bulk_results', allContacts); paiSave('bulk_filters', filters);
  ...
  function exportCSV() {
            ...
            results.forEach(p => rows.push([
                        ...
                        p.title||'', p.company_name||'', p.company_domain||'', p.email||'',
                        p.personal_phone||'', p.linkedin_url||'',
...
{LIMIT_OPTIONS.map(n => <option key={n} value={n}>{n}</option>)}
                   ...
                   {domains.length > 0 && <span style={{ fontSize:12, color:'#475569' }}>{domains.length} API calls ({domains.length} domains)</span>}
      </div>
         <button
          ...
          onClick={runSearch} disabled={stage === 1}
        >
...
{p.personal_phone
                     ? <a href={'tel:' + p.personal_phone} style={{ color:'#38bdf8', textDecoration:'none', fontWeight:500 }}>{p.personal_phone}</a>
                ...
                <button
                  ...
                  onClick={() => !hsSent[i] && pushHS(i)} disabled={hsSent[i]}
                >
