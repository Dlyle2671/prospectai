// v4 — userType support: independent / managed
...
function timeAgo(ts){if(!ts)return'—';const diff=Date.now()-new Date(ts).getTime();const m=Math.floor(diff/60000);if(m<1)return'just now';if(m<60)return m+'m ago';const h=Math.floor(m/60);if(h<24)return h+'h ago';const d=Math.floor(h/24);return d+'d ago';}
function fmtTTL(secs){if(!secs||secs<=0)return'expired';const h=Math.floor(secs/3600);const d=Math.floor(h/24);if(d>0)return d+'d '+(h%24)+'h';const m=Math.floor((secs%3600)/60);if(h>0)return h+'h '+m+'m';return m+'m';}
const s={
...
function UserDrawer({uid,onClose,onDelete}){
    ...
    useEffect(()=>{
          ...
          fetch('/api/admin?action=user&uid='+uid).then(r=>r.json()).then(d=>{setData(d);setLoading(false);setFlags(d.featureFlags||{});setUserType(d.userType||'independent');}).catch(()=>setLoading(false));
    },[uid]);
    async function saveFlags(){setSavingFlags(true);await fetch('/api/admin?action=flags&uid='+uid,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({flags})});setSavingFlags(false);setFlagsSaved(true);setTimeout(()=>setFlagsSaved(false),2000);}
    async function saveUserType(newType){setSavingType(true);setUserType(newType);await fetch('/api/admin?action=usertype&uid='+uid,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userType:newType})});setSavingType(false);setTypeSaved(true);setTimeout(()=>setTypeSaved(false),2000);}
    async function handleDelete(){setDeleting(true);const r=await fetch('/api/admin?action=user&uid='+uid,{method:'DELETE'});if(r.ok){onDelete(uid);onClose();}else{alert('Delete failed');setDeleting(false);}}
    return(
          <div style={s.drawer}>
...
{data.user.imageUrl?<img src={data.user.imageUrl} alt="" style={{width:52,height:52,borderRadius:'50%',objectFit:'cover'}}/>:<div style={{width:52,height:52,borderRadius:'50%',background:'#1e293b',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20}}>*</div>}
              ...
              <div style={{marginTop:4}}><span style={s.pill(userType==='managed'?'#8b5cf6':'#3b82f6')}>{userType==='managed'?'Managed':'Independent'}</span></div>
  </div>
  </div>
{uid!==ADMIN_USER_ID&&(
              <div style={{...s.card,padding:'14px 16px',marginBottom:16}}>
              <div style={{fontSize:12,color:'#475569',fontWeight:600,marginBottom:10,textTransform:'uppercase',letterSpacing:'.06em'}}>Account Type</div>
              <div style={{display:'flex',gap:10,marginBottom:10}}>
{['independent','managed'].map(t=>(
                    <button key={t} onClick={()=>saveUserType(t)} style={{flex:1,padding:'8px 0',borderRadius:8,border:'1px solid',borderColor:userType===t?(t==='managed'?'#8b5cf6':'#3b82f6'):'#1e293b',background:userType===t?(t==='managed'?'rgba(139,92,246,0.15)':'rgba(59,130,246,0.15)'):'transparent',color:userType===t?(t==='managed'?'#a78bfa':'#60a5fa'):'#475569',fontSize:12,fontWeight:600,cursor:'pointer'}}>
{t==='independent'?'Independent':'Managed'}
</button>
                ))}
                  </div>
              <div style={{fontSize:11,color:'#334155',lineHeight:1.5}}>{userType==='managed'?'Uses admin Apollo, ICP & integrations. Own sender email only.':'Sets up own Apollo, ICP & integrations.'}</div>
{typeSaved&&<div style={{fontSize:11,color:'#22c55e',marginTop:6}}>Account type saved</div>}
  </div>
          )}
          <div style={{...s.card,padding:'14px 16px',marginBottom:16}}>
          ...
          <div style={{...s.card,padding:'14px 16px',marginBottom:16}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:8,marginBottom:12}}>
              ...
              <span style={s.pill(userType==='managed'?'#8b5cf6':(data.icpWeights?'#22c55e':'#475569'))}>{userType==='managed'?'ICP inherited':(data.icpWeights?'ICP configured':'Default ICP')}</span>
              <span style={s.pill(userType==='managed'?'#8b5cf6':(data.integrations?.hubspot_token?'#22c55e':'#475569'))}>{userType==='managed'?'Integrations inherited':(data.integrations?.hubspot_token?'HubSpot connected':'No HubSpot')}</span>
{userType!=='managed'&&<span style={s.pill(data.integrations?.apollo_key?'#22c55e':'#475569')}>{data.integrations?.apollo_key?'Apollo':'No Apollo'}</span>}
  </div>
...
{!confirmDelete?<button style={s.btnDanger} onClick={()=>setConfirmDelete(true)}>Delete User</button>:(
              <div>
                  <div style={{fontSize:13,color:'#ef4444',marginBottom:12}}>This will permanently delete the user. Cannot be undone.</div>
...
const TOOLS=[{id:'leads',label:'Find Leads'},{id:'company',label:'Company Intel'},{id:'bulk',label:'Bulk Prospector'},{id:'jobchanges',label:'Job Changes'},{id:'peoplelookup',label:'People Lookup'},{id:'lookalike',label:'Lookalike'},{id:'awsopps',label:'Lead Scoring'},{id:'emailqueue',label:'Email Queue'},{id:'credits',label:'Credits'},{id:'salesanalytics',label:'Sales Analytics'}];
...
function InviteModal({onClose,onInvited}){
    ...
    async function handleSubmit(e){
          ...
          try{
                  const r=await fetch('/api/admin/invite',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email.trim(),flags,userType})});
                  const d=await r.json();
                  if(!r.ok){setError(d.error||'Invite failed');}
                  else{setSuccess('Invitation sent to '+email+'!');setEmail('');setFlags(defaultFlags);setUserType('independent');if(onInvited)onInvited(d);}
    ...
    return(
        ...
        <div style={{background:'#0a0f1e',border:'1px solid #1e293b',borderRadius:14,padding:28,width:500,maxHeight:'90vh',overflowY:'auto',boxSizing:'border-box'}} onClick={e=>e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{marginBottom:16}}>
            <label style={{display:'block',fontSize:12,color:'#94a3b8',fontWeight:600,marginBottom:8,textTransform:'uppercase',letterSpacing:'.06em'}}>Account Type</label>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
{[{value:'independent',icon:'Person',title:'Independent',desc:'Sets up own Apollo, ICP & integrations'},{value:'managed',icon:'Link',title:'Managed',desc:'Inherits your Apollo, ICP & integrations. Email-only setup.'}].map(opt=>(
                  <div key={opt.value} onClick={()=>setUserType(opt.value)} style={{padding:'12px 14px',borderRadius:10,border:'2px solid',borderColor:userType===opt.value?(opt.value==='managed'?'#8b5cf6':'#3b82f6'):'#1e293b',background:userType===opt.value?(opt.value==='managed'?'rgba(139,92,246,0.1)':'rgba(59,130,246,0.1)'):'#080c14',cursor:'pointer',transition:'all .15s'}}>
                  <div style={{fontSize:13,fontWeight:700,color:userType===opt.value?'#e2e8f0':'#64748b',marginBottom:4}}>{opt.title}</div>
                  <div style={{fontSize:11,color:'#475569',lineHeight:1.4}}>{opt.desc}</div>
  </div>
              ))}
                </div>
                </div>
...
{success&&<div style={{fontSize:13,color:'#22c55e',marginBottom:14,padding:'8px 12px',background:'rgba(20,83,45,0.13)',border:'1px solid rgba(20,83,45,0.33)',borderRadius:6}}>{success}</div>}
          <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
            ...
            <button type="submit" disabled={loading||!email.trim()} style={{padding:'8px 18px',borderRadius:7,border:'none',background:loading||!email.trim()?'#1e3a5f':'#1d4ed8',color:'#fff',fontSize:13,cursor:loading||!email.trim()?'not-allowed':'pointer',fontWeight:600}}>{loading?'Sending...':'Send Invite'}</button>
  </div>
  </form>
  </div>
  </div>
...
function IntentDashboard(){
    ...
    const load=useCallback(async()=>{setLoading(true);try{const r=await fetch('/api/admin/intent');const d=await r.json();setData(d);}catch(e){console.error(e);}setLoading(false);},[]);
    useEffect(()=>{load();},[load]);
    async function deleteDomain(domain){setDeletingDomain(domain);await fetch('/api/admin/intent?domain='+encodeURIComponent(domain),{method:'DELETE'});setData(prev=>({...prev,domains:prev.domains?.filter(d=>d.domain!==domain)||[]}));setDeletingDomain(null);}
    async function clearAll(){setClearing(true);if(data?.entries){await Promise.all(data.entries.map(d=>fetch('/api/admin/intent?domain='+encodeURIComponent(d.domain),{method:'DELETE'})));}setConfirmClear(false);setClearing(false);load();}
    async function seedDomainFn(){if(!seedDomain.trim())return;setSeeding(true);setSeedMsg('');try{const r=await fetch('/api/intent-webhook',{method:'POST',headers:{'Content-Type':'application/json','x-webhook-secret':'apollo-intent-2026'},body:JSON.stringify([{organization:{name:seedDomain.trim(),primary_domain:seedDomain.trim()},intent_strength:'HIGH',signals:['Manual seed'],source:'manual'}])});const d=await r.json();setSeedMsg(d.error?'Error: '+d.error:'Seeded '+seedDomain.trim());if(!d.error){setSeedDomain('');load();}}catch(e){setSeedMsg('Error: '+e.message);}setSeeding(false);}
    const limit=data?.daily?.limit||25;const todayCount=data?.daily?.count||0;const pct=Math.min(100,Math.round((todayCount/limit)*100));const history=data?.history||[];const maxHistCount=Math.max(1,...history.map(h=>h.count||0));const strengthColor={HIGH:'#22c55e',MEDIUM:'#f59e0b',LOW:'#64748b'};
    return(
          <div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        <div><div style={{fontSize:20,fontWeight:800,marginBottom:4}}>Intent Dashboard</div><div style={{fontSize:13,color:'#475569'}}>Apollo buying intent signals</div></div>
...
{!confirmClear?<button onClick={()=>setConfirmClear(true)} style={{...s.btnDanger,padding:'5px 14px'}}>Clear All</button>:(
...
{loading?<div style={{color:'#475569',padding:'40px 0',textAlign:'center'}}>Loading...</div>:(
        <>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:20}}>
            <div style={s.card}>
              <div style={{fontSize:12,color:'#475569',fontWeight:600,textTransform:'uppercase',letterSpacing:'.06em',marginBottom:12}}>Today Usage</div>
              <div style={{display:'flex',alignItems:'baseline',gap:8,marginBottom:10}}><span style={{fontSize:36,fontWeight:800,color:pct>=90?'#ef4444':pct>=70?'#f59e0b':'#22c55e'}}>{todayCount}</span><span style={{fontSize:16,color:'#475569'}}>/ {limit}</span></div>
                <div style={{background:'#1e293b',borderRadius:6,height:8,overflow:'hidden'}}><div style={{width:pct+'%',height:'100%',background:pct>=90?'#ef4444':pct>=70?'#f59e0b':'#22c55e',borderRadius:6}}/></div>
                <div style={{fontSize:11,color:'#475569',marginTop:6}}>{pct}% of daily limit</div>
  </div>
            <div style={s.card}>
...
{history.length===0?<div style={{fontSize:12,color:'#334155',fontStyle:'italic'}}>No history</div>:(
                <div style={{display:'flex',gap:6,alignItems:'flex-end',height:60}}>
{history.slice(-7).map((h,i)=>{const barH=Math.max(4,Math.round((h.count/maxHistCount)*52));const isToday=h.date===new Date().toISOString().slice(0,10);return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3}}><div style={{fontSize:9,color:'#475569'}}>{h.count}</div><div style={{width:'100%',height:barH,background:isToday?'#3b82f6':'#1e3a5f',borderRadius:3}}/><div style={{fontSize:8,color:'#334155'}}>{h.date?.slice(5)}</div></div>);})}
</div>
              )}
</div>
                </div>
...
{seedMsg&&<div style={{fontSize:12,marginTop:8,color:seedMsg.startsWith('Error')?'#ef4444':'#22c55e'}}>{seedMsg}</div>}
  </div>
          <div style={s.card}>
            <div style={{fontSize:15,fontWeight:700,marginBottom:16}}>Active Intent Domains ({data?.entries?.length||0})</div>
{!data?.entries?.length?<div style={{color:'#334155',fontStyle:'italic',fontSize:13}}>No intent domains yet.</div>:(
              <table style={s.table}>
                <thead><tr><th style={s.th}>Domain</th><th style={s.th}>Strength</th><th style={s.th}>Source</th><th style={s.th}>Updated</th><th style={s.th}></th></tr></thead>
                <tbody>
{data.entries.map((d,i)=>(
                      <tr key={d.domain} style={{background:i%2===0?'transparent':'rgba(255,255,255,.01)'}}>
                                        <td style={s.td}><div style={{fontWeight:600,fontSize:13}}>{d.org_name||d.domain}</div><div style={{fontSize:11,color:'#475569'}}>{d.domain}</div></td>
                                        ...
                                        <td style={s.td}><button style={s.btnDanger} onClick={()=>deleteDomain(d.domain)} disabled={deletingDomain===d.domain}>{deletingDomain===d.domain?'...':'Del'}</button></td>
  </tr>
                  ))}
                    </tbody>
                    </table>
...
export default function AdminPortal(){
    ...
    useEffect(()=>{if(!isLoaded)return;if(!isAdmin){router.replace('/');return;}loadUsers();},[isLoaded,isAdmin]);
    const loadUsers=useCallback(async()=>{setLoading(true);try{const[usersRes,invitesRes]=await Promise.all([fetch('/api/admin?action=users'),fetch('/api/admin/invite')]);const usersData=await usersRes.json().catch(()=>({users:[]}));const invitesData=await invitesRes.json().catch(()=>({invites:[]}));setUsers(usersData.users||[]);setInvites(invitesData.invites||[]);}catch(err){console.error('[loadUsers]',err);}setLastRefresh(new Date());setLoading(false);},[]);
    async function revokeInvite(inviteId){await fetch('/api/admin/invite?inviteId='+inviteId,{method:'DELETE'});setInvites(prev=>prev.filter(i=>i.id!==inviteId));}
    ...
    const stats={total:users.length,onboarded:users.filter(u=>u.onboarded).length,activeToday:users.filter(u=>u.lastSignInAt&&(Date.now()-new Date(u.lastSignInAt))<86400000).length,managed:users.filter(u=>u.userType==='managed').length,independent:users.filter(u=>!u.userType||u.userType==='independent').length};
  ...
  const tabBtn=(active)=>({padding:'6px 16px',borderRadius:7,border:'none',fontSize:13,fontWeight:600,cursor:'pointer',background:active?'#1e3a5f':'transparent',color:active?'#60a5fa':'#475569',borderBottom:active?'2px solid #3b82f6':'2px solid transparent'});
  return(
        <>
          ...
          <div style={{display:'flex',gap:4,marginLeft:16}}>
            ...
            <button style={tabBtn(activeTab==='intent')} onClick={()=>setActiveTab('intent')}>Intent</button>
    </div>
...
{[{label:'Total Users',value:stats.total,color:'#3b82f6'},{label:'Onboarded',value:stats.onboarded,color:'#22c55e'},{label:'Active Today',value:stats.activeToday,color:'#f59e0b'},{label:'Managed',value:stats.managed,color:'#8b5cf6'},{label:'Independent',value:stats.independent,color:'#0ea5e9'}].map(({label,value,color})=>(
                    <div key={label} style={s.stat}><div style={{...s.statNum,color}}>{loading?'—':value}</div><div style={s.statLabel}>{label}</div></div>
                ))}
                  </div>
              <div style={s.card}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                  <div style={{fontSize:15,fontWeight:700}}>Users ({filtered.length})</div>
                  <input placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)} style={{background:'#080c14',border:'1px solid #1e293b',borderRadius:8,color:'#e2e8f0',fontSize:13,padding:'7px 12px',width:260,outline:'none'}}/>
                  </div>
{loading?<div style={{color:'#475569',padding:'32px 0',textAlign:'center'}}>Loading...</div>:(
                  <table style={s.table}>
                    <thead><tr><th style={s.th}>User</th><th style={s.th}>Type</th><th style={s.th}>Joined</th><th style={s.th}>Last Active</th><th style={s.th}>Status</th><th style={s.th}></th></tr></thead>
                      <tbody>
{filtered.map(u=>(
...
{u.imageUrl?<img src={u.imageUrl} alt="" style={{width:32,height:32,borderRadius:'50%',objectFit:'cover',flexShrink:0}}/>:<div style={{width:32,height:32,borderRadius:'50%',background:'#1e293b',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,flexShrink:0}}>*</div>}
                                <div><div style={{fontWeight:600,fontSize:13}}>{u.firstName} {u.lastName}{u.id===ADMIN_USER_ID?' (admin)':''}</div><div style={{fontSize:11,color:'#475569'}}>{u.email}</div></div>
  </div>
  </td>
                          <td style={s.td}><span style={s.pill(u.userType==='managed'?'#8b5cf6':'#3b82f6')}>{u.userType==='managed'?'Managed':'Independent'}</span></td>
...
{u.hasSenderEmail&&<span style={s.pill('#0ea5e9')}>Email</span>}
...
{invites.length>0&&(
                  ...
                  <table style={s.table}>
                    <thead><tr><th style={s.th}>Email</th><th style={s.th}>Type</th><th style={s.th}>Invited</th><th style={s.th}>Status</th><th style={s.th}></th></tr></thead>
                    <tbody>
{invites.map((inv,i)=>(
                          <tr key={inv.id||i}>
                                       <td style={s.td}>{inv.email}</td>
                                       <td style={s.td}><span style={s.pill(inv.userType==='managed'?'#8b5cf6':'#3b82f6')}>{inv.userType==='managed'?'Managed':'Independent'}</span></td>
...
{showInviteModal&&(
          <InviteModal onClose={()=>setShowInviteModal(false)} onInvited={(d)=>{setInvites(prev=>[{id:d.invitationId,email:d.email,status:'pending',createdAt:new Date().toISOString(),userType:d.userType},...prev]);}}/>
      )}
</>
  );
}
