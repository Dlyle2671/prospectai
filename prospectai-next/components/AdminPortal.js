...
function UserDrawer({ uid, onClose, onDelete }) {
  ...
  async function saveFlags() {
    setSavingFlags(true);
    await fetch('/api/admin?action=flags&uid=' + uid, {
      method: 'POST',
        ...
        <>
          ...
          <div style={{ borderTop: '1px solid #1e293b', paddingTop: 20, marginBottom: 20 }}>
              ...
              {[
                ...
                return (
                  <label key={tool.id} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: enabled ? '#f1f5f9' : '#475569' }}>
                    <div
                      onClick={() => setFlags(prev => ({ ...prev, [tool.id]: !enabled }))}
                      style={{ width: 36, height: 20, borderRadius: 10, flexShrink: 0, background: enabled ? '#6366f1' : '#1e293b', border: '1px solid ' + (enabled ? '#818cf8' : '#334155'), position: 'relative', cursor: 'pointer', transition: 'background .2s' }}
                    >
            ...
            <button
              ...
              style={{ marginTop: 14, padding: '7px 18px', background: flagsSaved ? 'rgba(34,197,94,.15)' : 'rgba(99,102,241,.2)', border: '1px solid ' + (flagsSaved ? '#22c55e' : 'rgba(99,102,241,.4)'), color: flagsSaved ? '#22c55e' : '#a5b4fc', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
            >
