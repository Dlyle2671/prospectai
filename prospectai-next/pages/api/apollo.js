...
// Reads Apollo API key from user's own Redis-stored integrations first, then falls back to env var
...
async function enrichPerson(p, apiKey, searchedDomain, icp) {
  try {
    ...
    if(!r.ok){const errBody=await r.json().catch(()=>({})); console.log('[enrich] status',r.status,'for',p.first_name,p.last_name,'| error:',(errBody.message||errBody.error||'').substring(0,80)); const orgDomain=searchedDomain||p.organization?.primary_domain||p.organization?.domain||null; const enrichedOrg=await enrichOrg(orgDomain,apiKey); return buildLead(p,enrichedOrg||p.organization||{},icp);}
    const d=await r.json();
    const ep=d.person;
    if(!ep){const orgDomain=searchedDomain||p.organization?.primary_domain||null; const enrichedOrg=await enrichOrg(orgDomain,apiKey); return buildLead(p,enrichedOrg||p.organization||{},icp);}
  ...
  } catch(err){console.log('[enrich] error for',p.id,':',err.message); return buildLead(p,p.organization||{},icp);}
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) return res.status(400).json({ error: 'Apollo API key not configured. Add it in Settings → Integrations.' });
    ...
    if(isDomainSearch){const domain=organization_domains[0]; const orgId=await getOrgIdByDomain(domain,apiKey); if(orgId) body.organization_ids=[orgId]; else body.organization_domains=organization_domains; body.contact_email_status=['verified','guessed','unavailable','bounced'];}
    else {body.contact_email_status=['verified','guessed']; body.organization_technology_names=['Amazon Web Services'].concat((tech_stack||[]).filter(t=>t!=='AWS'&&t!=='Amazon Web Services'));}
    ...
    if(!isDomainSearch){if(industry.length>0) body.q_organization_industry_tag_ids=industry; if(req.body.industries?.length>0) body.q_keywords=req.body.industries.join(' ');}
    const sizeRanges=req.body.organization_num_employees_ranges||employee_ranges||[];
    if(sizeRanges.length>0){const map={'1-10':'1,10','11-25':'11,25','26-50':'26,50','51-100':'51,100','101-250':'101,250','251+':'251,10000000','1-50':'1,50','11-50':'11,50','51-200':'51,200','201-500':'201,500','501-1000':'501,1000','1001-5000':'1001,5000','5001+':'5001,10000000','1,10':'1,10','11,50':'11,50','51,200':'51,200','201,500':'201,500','501,1000':'501,1000','1001,5000':'1001,5000','5001,10000000':'5001,10000000'}; const mapped=sizeRanges.map(r=>map[r]).filter(Boolean); if(mapped.length>0) body.organization_num_employees_ranges=mapped;}
    ...
    if (domains.length > 0) {
      ...
      const intentMap = await batchFetchIntentStrength(domains, apiKey);
        ...
        if (intentData) {
          ...
          (intentData.intent_signals || []).forEach(s => {
            ...
            if (label && !existingLabels.has(label)) {
              lead.intent_signals.push({ type: 'intent', label });
            }
          });
        }
      });
