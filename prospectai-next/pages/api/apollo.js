// ProspectAI — /api/apollo // Full lead scoring + Apollo enrichment pipeline
const DEFAULT_ICP = {
    titleWeight: 40,
    seniorityWeight: 30,
...
function calcLeadScore(lead, icp) {
    ...
    if(titleRatio < 0.5) { const sl=(lead.seniority||'').toLowerCase(); for(const [k,r] of Object.entries(SENIORITY_RATIO)) if(sl.includes(k)){score+=Math.round(w.seniorityWeight*r);break;} }
...
async function enrichPerson(p, apiKey, searchedDomain, icp) {
    try {
          const r = await fetch('https://api.apollo.io/api/v1/people/match', { method:'POST', headers:{'Content-Type':'application/json','X-Api-Key':apiKey}, body:JSON.stringify({ id: p.id, reveal_personal_emails: true, reveal_phone_number: true }) });
          ...
          const lead = {
                  id:p.id, name:ep.name||p.name||'', first_name:ep.first_name||p.first_name||'', last_name:ep.last_name||p.last_name||'', title:ep.title||p.title||'', seniority:ep.seniority||p.seniority||'', department:ep.departments?.[0]||ep.department||p.departments?.[0]||'', email, email_status:ep.email_status||ep.contact_email_status||'', linkedin_url:ep.linkedin_url||'', twitter_url:ep.twitter_url||'', github_url:ep.github_url||'', photo_url:ep.photo_url||'', city:ep.city||p.city||'', state:ep.state||p.state||'', country:ep.country||p.country||'',
                  ...
                  company_name:org.name||p.organization?.name||'', company_domain:searchedDomain||org.primary_domain||p.organization?.primary_domain||'', company_industry:org.industry||p.organization?.industry||'', subindustry:org.subindustry||'', company_size:org.estimated_num_employees||p.organization?.estimated_num_employees||'', company_founded:org.founded_year||'', company_linkedin:org.linkedin_url||'', company_twitter:org.twitter_url||'', company_description:(org.short_description||'').slice(0,160), seo_description:org.seo_description||'',
                  personal_phone, all_phones, company_phone:org.primary_phone?.number||org.phone||null, company_street:org.street_address||null, company_zip:org.postal_code||null, company_city:org.city||null, company_state:org.state||null, company_country:org.country||null,
                  market_cap:org.market_cap||null, tech_stack:techStack, aws_services, funding_stage, funding_total:org.total_funding||null, funding_round_date, funding_round_type, funding_round_amount, top_investors, num_funding_rounds:org.num_funding_rounds||fundingEvents.length||null,
                  headcount_growth, linkedin_follower_count:org.linkedin_follower_count||null, annual_revenue:fmtRevenue(org.annual_revenue_printed||org.annual_revenue||null), alexa_rank:org.alexa_rank||null, job_postings_count, keywords:(org.keywords||[]).slice(0,5), hiring_surge:hiring_surge||false, recently_funded, intent_signals,
          };
...
export default async function handler(req, res) {
    ...
    try {
          ...
          if(email||linkedin_url) {
                  ...
                  const all_phones = Array.isArray(phoneNums)?phoneNums.map(x=>x.sanitized_number||x.raw_number||x.number||'').filter(Boolean):(personal_phone?[personal_phone]:[]);
                  const lead = {
                            id:p.id, name:p.name||'', first_name:p.first_name||'', last_name:p.last_name||'', title:p.title||'', seniority:p.seniority||'', department:p.departments?.[0]||'', email:p.email||'', email_status:p.email_status||'', linkedin_url:p.linkedin_url||'', twitter_url:p.twitter_url||'', github_url:p.github_url||'', photo_url:p.photo_url||'', city:p.city||'', state:p.state||'', country:p.country||'',
                            company_name:org.name||'', company_domain:org.primary_domain||'', company_industry:org.industry||'', company_size:org.estimated_num_employees||'', company_founded:org.founded_year||'', company_linkedin:org.linkedin_url||'', company_description:(org.short_description||'').slice(0,160), seo_description:org.seo_description||'',
          ...
          if(sizeRanges.length>0) {
                  const map={ '1-10':'1,10','11-25':'11,25','26-50':'26,50','51-100':'51,100','101-250':'101,250','251+':'251,10000000','1-50':'1,50','11-50':'11,50','51-200':'51,200','201-500':'201,500','501-1000':'501,1000','1001-5000':'1001,5000','5001+':'5001,10000000','1,10':'1,10','11,50':'11,50','51,200':'51,200','201,500':'201,500','501,1000':'501,1000','1001,5000':'1001,5000','5001,10000000':'5001,10000000' };
    ...
    } catch(err) { console.error('[apollo] error:',err.message); return res.status(500).json({error:err.message}); }
}
