...
const DEFAULT_ICP = {
      titleWeight: 40, seniorityWeight: 30, companySizeWeight: 25, industryWeight: 30,
      fundingWeight: 15, verifiedEmailBonus: 3, linkedinBonus: 2, phoneBonus: 2,
...
// Build a lead object from raw search data (no enrichment call needed)
function buildLeadFromSearch(p, icp) {
      ...
      const lead = {
              id:p.id, name, first_name:p.first_name||'', last_name:p.last_name||'',
              title:p.title||'', seniority:p.seniority||'',
              department:p.departments?.[0]||p.department||'',
              email:p.email||'', email_status:p.email_status||p.contact_email_status||'',
              linkedin_url:p.linkedin_url||'', twitter_url:p.twitter_url||'',
              github_url:p.github_url||'', photo_url:p.photo_url||'',
              ...
              personal_phone, all_phones,
              company_phone:org.primary_phone?.number||org.phone||null,
              company_street:org.street_address||null, company_zip:org.postal_code||null,
              company_city:org.city||null, company_state:org.state||null, company_country:org.country||null,
              market_cap:org.market_cap||null, tech_stack:techStack, aws_services,
              funding_stage, funding_total:org.total_funding||null,
              funding_round_date, funding_round_type, funding_round_amount, top_investors,
              ...
              alexa_rank:org.alexa_rank||null, job_postings_count,
              keywords:(org.keywords||[]).slice(0,5),
              hiring_surge:hiring_surge||false, recently_funded, intent_signals,
      };
...
async function enrichPerson(p, apiKey, searchedDomain, icp) {
      try {
              ...
              if(!r.ok) {
                        console.log('[enrich] non-ok status', r.status, 'for', p.id, '- using search data');
                        return buildLeadFromSearch(p, icp);
              }
              const d = await r.json();
              const ep = d.person;
              if(!ep) {
                        console.log('[enrich] no person returned for', p.id, '- using search data');
                        return buildLeadFromSearch(p, icp);
              }
              ...
              if(recently_funded&&funding_stage) intent_signals.push({type:'funding',label:funding_stage+(funding_round_amount?(funding_round_amount>=1e9?' $'+(funding_round_amount/1e9).toFixed(1)+'B':funding_round_amount>=1e6?' $'+(funding_round_amount/1e6).toFixed(0)+'M':''):'')}); else if(recently_funded) intent_signals.push({type:'funding',label:'Recently Funded'});
              if(hiring_surge) intent_signals.push({type:'hiring',label:job_postings_count?job_postings_count+' open roles':'Hiring Surge'});
              const name = ep.name||[ep.first_name,ep.last_name].filter(Boolean).join(' ')||p.name||[p.first_name,p.last_name].filter(Boolean).join(' ')||'';
              const lead = {
                        id:p.id, name, first_name:ep.first_name||p.first_name||'', last_name:ep.last_name||p.last_name||'',
                        title:ep.title||p.title||'', seniority:ep.seniority||p.seniority||'',
                        department:ep.departments?.[0]||ep.department||p.departments?.[0]||'',
                        email, email_status:ep.email_status||ep.contact_email_status||'',
                        linkedin_url:ep.linkedin_url||'', twitter_url:ep.twitter_url||'',
                        github_url:ep.github_url||'', photo_url:ep.photo_url||'',
                        ...
                        company_industry:org.industry||p.organization?.industry||'',
                        subindustry:org.subindustry||'', company_size:org.estimated_num_employees||p.organization?.estimated_num_employees||'',
                        company_founded:org.founded_year||'', company_linkedin:org.linkedin_url||'', company_twitter:org.twitter_url||'',
                        company_description:(org.short_description||'').slice(0,160), seo_description:org.seo_description||'',
                        personal_phone, all_phones, company_phone:org.primary_phone?.number||org.phone||null,
                        company_street:org.street_address||null, company_zip:org.postal_code||null,
                        company_city:org.city||null, company_state:org.state||null, company_country:org.country||null,
                        market_cap:org.market_cap||null, tech_stack:techStack, aws_services,
                        funding_stage, funding_total:org.total_funding||null, funding_round_date, funding_round_type,
                        funding_round_amount, top_investors, num_funding_rounds:org.num_funding_rounds||fundingEvents.length||null,
                        headcount_growth, linkedin_follower_count:org.linkedin_follower_count||null,
                        annual_revenue:fmtRevenue(org.annual_revenue_printed||org.annual_revenue||null),
                        alexa_rank:org.alexa_rank||null, job_postings_count, keywords:(org.keywords||[]).slice(0,5),
                        hiring_surge:hiring_surge||false, recently_funded, intent_signals,
              };
              lead.score = calcLeadScore(lead, icp);
              lead.score_label = scoreLabel(lead.score, icp);
              ...
              console.log('[enrich] error for', p.id, ':', err.message, '- falling back to search data');
              return buildLeadFromSearch(p, icp);
      }
}
...
export default async function handler(req, res) {
      ...
      try {
        ...
        if(email||linkedin_url) {
                  ...
                  const lead = buildLeadFromSearch(p, icp);
        ...
        const body = { api_key:apiKey, page, per_page };
              ...
              const pagination = searchData.pagination||searchData.meta||{};
              console.log('[apollo] total_entries=', pagination.total_entries, 'people=', searchData.people?.length, 'error=', searchData.error);
              ...
              const candidates = isDomainSearch ? people : people.filter(p=>p.email||p.has_email);
        ...
        const results = [];
              for(let i=0; i<candidates.length; i+=5) {
                        ...
                        const enriched = await Promise.all(batch.map(p=>enrichPerson(p, apiKey, isDomainSearch?organization_domains[0]:null, icp)));
                        results.push(...enriched);
                        if(i+5 < candidates.length) await new Promise(r=>setTimeout(r, 300));
              }
              return res.status(200).json(results.filter(Boolean).sort((a,b)=>b.score-a.score));
      } catch(err) {
