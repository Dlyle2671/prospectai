...
function buildLead(p, org, icp) {
            ...
            const lead = {
                          ...
                          company_industry, subindustry:org.subindustry||'', company_size,
                          company_founded:org.founded_year||'',
                          company_linkedin:org.linkedin_url||'', company_twitter:org.twitter_url||'',
                          company_description:(org.short_description||'').slice(0,160),
                          seo_description:org.seo_description||'', personal_phone, all_phones,
...
async function enrichOrg(domain, apiKey) {
            ...
            try {
                          ...
                          if(!r.ok) { console.log('[org-enrich] status', r.status, 'for', domain); return null; }
                          const d = await r.json();
                          const o = d.organization||null;
                          if(o) console.log('[org-enrich] ok:', o.name, '| industry:', o.industry, '| size:', o.estimated_num_employees);
                          return o;
            } catch(e) { console.log('[org-enrich] error:', e.message); return null; }
}
async function enrichPerson(p, apiKey, searchedDomain, icp) {
            try {
                          // NOTE: reveal_phone_number requires webhook_url param — omit it to avoid 400 errors
              const matchBody = { api_key:apiKey, reveal_personal_emails:true };
                          ...
                          if(!r.ok) {
                                          ...
                                          console.log('[enrich] status', r.status, 'for', p.first_name, p.last_name, '| error:', (errBody.message||errBody.error||'').substring(0,80));
                                          const orgDomain = searchedDomain||p.organization?.primary_domain||p.organization?.domain||null;
                                          const enrichedOrg = await enrichOrg(orgDomain, apiKey);
                                          return buildLead(p, enrichedOrg||p.organization||{}, icp);
                          }
                          const d = await r.json();
                          const ep = d.person;
                          if(!ep) {
                                          const orgDomain = searchedDomain||p.organization?.primary_domain||null;
                                          const enrichedOrg = await enrichOrg(orgDomain, apiKey);
                                          return buildLead(p, enrichedOrg||p.organization||{}, icp);
                          }
                          const org = ep.organization||p.organization||{};
                          console.log('[enrich] ok:', ep.name, '| industry:', org.industry, '| size:', org.estimated_num_employees);
                          return buildLead(ep, org, icp);
            } catch(err) {
                          console.log('[enrich] error for', p.id, ':', err.message);
                          return buildLead(p, p.organization||{}, icp);
            }
}
...
export default async function handler(req, res) {
            ...
            try {
                          const {
                                          industry=[], employee_ranges=[], tech_stack=[], titles=[], locations=[],
                                          page=1, per_page=25, email, linkedin_url, changed_jobs_recently,
                                          organization_domains=[], icp_weights
                          } = req.body;
                          ...
                          if(email||linkedin_url) {
                                          ...
                                          return res.status(200).json([buildLead(p, p.organization, icp)]);
                          }
                          ...
                          if(isDomainSearch) {
                                          ...
                                          else body.organization_domains=organization_domains;
                          ...
                          console.log('[apollo] total_entries=', pagination.total_entries, 'people=', searchData.people?.length);
                          ...
                          console.log('[apollo] candidates:', candidates.length);
