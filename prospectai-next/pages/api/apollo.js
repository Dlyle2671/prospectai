...
// (Copied verbatim from prospectai/api/apollo.js — already Next.js compatible)
...
export default async function handler(req, res) {
  ...
  try {
    const { industry=[], employee_ranges=[], tech_stack=[], titles=[], locations=[], page=1, per_page=25, email, linkedin_url, changed_jobs_recently } = req.body;
    const apiKey = process.env.APOLLO_API_KEY;

    // Email/LinkedIn lookup mode
    if(email || linkedin_url) {
      const matchBody = { api_key: apiKey };
      ...
      const lead = {
        ...
        company_name:org.name||'', company_domain:org.primary_domain||'', company_industry:org.industry||'', company_size:org.estimated_num_employees||'',
        company_founded:org.founded_year||'', company_linkedin:org.linkedin_url||'', company_description:(org.short_description||'').slice(0,160), seo_description:org.seo_description||'',
        personal_phone:directDial?.sanitized_number||p.mobile_phone||p.phone||null,
      ...
      lead.score = calcLeadScore(lead); lead.score_label = scoreLabel(lead.score);
      return res.status(200).json([lead]);
    }

    // Search mode
    const body = {
      api_key: apiKey, page, per_page,
      contact_email_status:['verified','guessed'],
      organization_technology_names:['Amazon Web Services'].concat((tech_stack||[]).filter(t=>t!=='AWS'&&t!=='Amazon Web Services')),
    };
    if(industry.length>0) body.q_organization_industry_tag_ids = industry;
    if(titles.length>0) body.person_titles = titles;
    if(req.body.person_titles?.length>0) body.person_titles = req.body.person_titles;
    ...
    if(req.body.organization_num_employees_ranges?.length>0) body.organization_num_employees_ranges = req.body.organization_num_employees_ranges;
    ...
    const enriched = await Promise.all(candidates.map(async(p) => {
      try {
        const r = await fetch('https://api.apollo.io/api/v1/people/match', { method:'POST', headers:{'Content-Type':'application/json','X-Api-Key':apiKey}, body:JSON.stringify({id:p.id}) });
        const d = await r.json();
        const ep = d.person||{};
        ...
        const lead = {
          ...
          linkedin_url:ep.linkedin_url||'', twitter_url:ep.twitter_url||'', github_url:ep.github_url||'', photo_url:ep.photo_url||'',
          ...
          personal_phone:directDial?.sanitized_number||directDial?.number||ep.mobile_phone||ep.phone||null,
          company_phone:org.primary_phone?.number||org.phone||null,
          ...
          tech_stack:techStack, aws_services,
          funding_stage, funding_total:org.total_funding||null, funding_round_date, funding_round_type, funding_round_amount,
          top_investors, num_funding_rounds:org.num_funding_rounds||fundingEvents.length||null,
