...
async function enrichPerson(p, apiKey, searchedDomain) {
    try {
          ...
          const ep = d.person||{};
          // Always prefer enriched org data; fall back to search result org
...
export default async function handler(req, res) {
  ...
  try {
      ...
      if(isDomainSearch) {
              // Domain search: Apollo filters by domain server-side — trust its results directly
      ...
      const searchResp = await fetch('https://api.apollo.io/api/v1/mixed_people/api_search', {
        ...
        if(searchData.people?.length > 0) {
                ...
                console.log('[apollo] sample person:', JSON.stringify({name:fp.name, title:fp.title, org_name:fp.organization?.name, org_domain:fp.organization?.primary_domain, has_email:fp.has_email, email_status:fp.contact_email_status}));
        }
      ...
      // For domain searches, Apollo already filters by domain — no need to re-filter
      // For regular searches, just require has_email
