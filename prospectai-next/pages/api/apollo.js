...
async function enrichPerson(p, apiKey, searchedDomain) {
                try {
                                        ...
                                        const ep = d.person||{};
                                        // Always prefer enriched org data; fall back to search result org if enrichment lacks it
...
export default async function handler(req, res) {
        ...
        try {
                        ...
                        const candidates = isDomainSearch
                                        ? (searchData.people||[]).filter(p => { const org = p.organization||{}; const pd = (org.primary_domain||org.website_url||org.domain||'').replace(/^https?:\/\/(www\.)?/,'').split('/')[0].toLowerCase(); const sd = (organization_domains[0]||'').toLowerCase(); const match = !sd || pd === sd || pd.endsWith('.'+sd) || sd.endsWith('.'+pd); console.log('[domain-filter] searched='+sd+' person_org_domain='+pd+' match='+match); return match && (p.email || p.has_email || p.contact_email_status !== 'notFound'); })
