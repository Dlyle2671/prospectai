...
// Build a lead object from raw person data + org data
...
// Enrich org by domain to get industry, size, tech stack
...
async function enrichPerson(p, apiKey, searchedDomain, icp) {
          try {
                      ...
                      if(!r.ok) {
                                    ...
                                    console.log('[enrich] status', r.status, 'for', p.first_name, p.last_name, '| error:', errBody.message||errBody.error||JSON.stringify(errBody).substring(0,100));
                                    // Fallback: try to enrich the org to get industry/size
