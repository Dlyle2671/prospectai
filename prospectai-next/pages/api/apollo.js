...
function calcLeadScore(lead) { let score=0; const tl=(lead.title||'').toLowerCase(); let tp=0; for(const [k,p] of Object.entries(TITLE_SCORE)) if(tl.includes(k)) tp=Math.max(tp,p); score+=tp; if(tp<20){ const sl=(lead.seniority||'').toLowerCase(); for(const [k,p] of Object.entries(SENIORITY_SCORE)) if(sl.includes(k)){score+=p;break;} } score+=scoreEmployeeCount(lead.company_size); const il=(lead.company_industry||'').toLowerCase(); score+=HIGH_VALUE_INDUSTRIES.some(i=>il.includes(i))?20:5; if(lead.linkedin_url) score+=2; if(lead.email_status==='verified') score+=3; if(lead.personal_phone) score+=2; if(lead.funding_round_date){ const mo=(Date.now()-new Date(lead.funding_round_date))/(1000*60*60*24*30); if(mo<=18) score+=8; } if(lead.linkedin_follower_count>1000) score+=3; if(lead.job_postings_count>5) score+=2; if(lead.twitter_url) score+=1; if(lead.hiring_surge) score+=3; if(lead.aws_services?.length>=3) score+=3; return Math.min(100,Math.round(score)); }
...
// Look up Apollo org ID by domain using organizations/enrich
      ...
      try {
        ...
        if(email || linkedin_url) {
                  ...
                  const lead = {
                              ...
                              email:p.email||'', email_status:p.email_status||'', linkedin_url:p.linkedin_url||'',
                              twitter_url:p.twitter_url||'', github_url:p.github_url||'', photo_url:p.photo_url||'',
                              city:p.city||'', state:p.state||'', country:p.country||'',
                              company_name:org.name||'', company_domain:org.primary_domain||'',
                              company_industry:org.industry||'', company_size:org.estimated_num_employees||'',
                              company_founded:org.founded_year||'', company_linkedin:org.linkedin_url||'',
        ...
        if(isDomainSearch) {
                  ...
                  // Get the Apollo org ID for this domain to ensure exact company matching
                const orgId = await getOrgIdByDomain(domain, apiKey);
                  if(orgId) {
                              // Filter by org ID — guarantees results are from this exact company
                    body.organization_ids = [orgId];
                  } else {
                              // Fallback to domain param if org lookup fails
        ...
        console.log('[apollo] search body:', JSON.stringify(body));
              ...
              if(searchData.people?.length > 0) {
                        const fp = searchData.people[0];
                        console.log('[apollo] sample person:', JSON.stringify({name:fp.name, title:fp.title, org_name:fp.organization?.name, org_domain:fp.organization?.primary_domain}));
              }
