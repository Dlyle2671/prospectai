...
// Build a lead object from raw person data (search result or enriched)
function buildLead(p, org, icp) {
        ...
        // company size: try multiple field names Apollo may use
  const company_size = org.estimated_num_employees||org.num_employees||org.headcount||p.organization?.estimated_num_employees||null;
        // industry: try multiple field names
  const company_industry = org.industry||org.primary_industry||p.industry||p.organization?.industry||'';
        const lead = {
                  id:p.id,
                  name,
                  ...
                  email:p.email||'',
                  email_status:p.email_status||p.contact_email_status||'',
...
async function enrichPerson(p, apiKey, searchedDomain, icp) {
        try {
                  // Build match body with as many identifiers as possible for better match rate
          const matchBody = { reveal_personal_emails:true, reveal_phone_number:true };
                  ...
                  if(!r.ok) {
                              console.log('[enrich] status', r.status, 'for', p.id, p.first_name, p.last_name, '- using search data');
                              return buildLead(p, p.organization||p.account, icp);
                  }
                  const d = await r.json();
                  const ep = d.person;
                  if(!ep) {
                              console.log('[enrich] no person in response for', p.id, '- using search data');
                              return buildLead(p, p.organization||p.account, icp);
                  }
                  const org = ep.organization||p.organization||p.account||{};
                  console.log('[enrich] ok for', ep.name, '| industry:', org.industry, '| size:', org.estimated_num_employees, '| domain:', searchedDomain||org.primary_domain);
                  return buildLead(ep, org, icp);
        } catch(err) {
                  console.log('[enrich] error for', p.id, ':', err.message, '- using search data');
                  return buildLead(p, p.organization||p.account, icp);
        }
}
async function getOrgIdByDomain(domain, apiKey) {
        try {
                  ...
                  console.log('[org-enrich] error for domain='+domain+': '+e.message);
                  return null;
        }
}
export default async function handler(req, res) {
        ...
        try {
                  ...
                  if(email||linkedin_url) {
                              ...
                              const lead = buildLead(p, p.organization, icp);
                  ...
                  // Log first person's org data to debug field names
          if(people.length>0) {
                      const fp = people[0];
                      const fo = fp.organization||fp.account||{};
                      console.log('[apollo] first person org keys:', Object.keys(fo).join(','));
                      console.log('[apollo] first person:', fp.name||fp.first_name, '| industry:', fo.industry, '| size:', fo.estimated_num_employees, '| email:', fp.email);
          }
