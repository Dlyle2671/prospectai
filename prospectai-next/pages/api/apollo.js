...
export default async function handler(req, res) {
  ...
  try {
        const {
                ...
                page=1, per_page=25,
                email, linkedin_url, changed_jobs_recently,
      ...
      if(email || linkedin_url) {
              ...
              const lead = {
                        ...
                        linkedin_url:p.linkedin_url||'', twitter_url:p.twitter_url||'', github_url:p.github_url||'',
                        photo_url:p.photo_url||'', city:p.city||'', state:p.state||'', country:p.country||'',
      ...
      // Domain-based filtering (Bulk Prospector)
      if(organization_domains.length>0) {
              body.q_organization_domains = organization_domains;
              // When filtering by domain, remove tech stack requirement to maximise results
          delete body.organization_technology_names;
      }
      ...
      const enriched = await Promise.all(candidates.map(async(p) => {
              try {
                ...
                const lead = {
                            ...
                            linkedin_url:ep.linkedin_url||'', twitter_url:ep.twitter_url||'', github_url:ep.github_url||'',
                            photo_url:ep.photo_url||'', city:ep.city||p.city||'', state:ep.state||p.state||'', country:ep.country||p.country||'',
                            prev_jobs:prevJobs,
