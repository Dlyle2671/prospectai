...
const DEFAULT_ICP = {
        titleWeight: 40, seniorityWeight: 30, companySizeWeight: 35, industryWeight: 20, fundingWeight: 8,
        verifiedEmailBonus: 3, linkedinBonus: 2, phoneBonus: 2, hiringSurgeBonus: 3, awsBonus: 3,
        hotThreshold: 75, warmThreshold: 50,
...
function scoreEmployeeCount(count, maxPts) {
        if(!count) return 0; const n=Number(count);
        if(n>=501&&n<=5000) return maxPts;
        if(n>=201&&n<=500) return Math.round(maxPts*0.86);
        if(n>=51&&n<=200) return Math.round(maxPts*0.63);
        if(n>5000) return Math.round(maxPts*0.71);
        if(n>=11&&n<=50) return Math.round(maxPts*0.34);
        return Math.round(maxPts*0.14);
}

function calcLeadScore(lead, icp) {
        const w = { ...DEFAULT_ICP, ...(icp||{}) };
        let score = 0;
        const tl = (lead.title||'').toLowerCase();
        let titleRatio = 0;
        for(const [k,r] of Object.entries(TITLE_RATIO)) if(tl.includes(k)) titleRatio=Math.max(titleRatio,r);
        score += Math.round(w.titleWeight * titleRatio);
        if(titleRatio < 0.5) {
                  const sl=(lead.seniority||'').toLowerCase();
                  for(const [k,r] of Object.entries(SENIORITY_RATIO)) if(sl.includes(k)){score+=Math.round(w.seniorityWeight*r);break;}
        }
        score += scoreEmployeeCount(lead.company_size, w.companySizeWeight);
        const il=(lead.company_industry||'').toLowerCase();
        const inds = Array.isArray(w.targetIndustries)?w.targetIndustries:DEFAULT_ICP.targetIndustries;
        score += inds.some(i=>il.includes(i)) ? w.industryWeight : 5;
        if(lead.linkedin_url) score+=w.linkedinBonus;
        if(lead.email_status==='verified') score+=w.verifiedEmailBonus;
        if(lead.personal_phone) score+=w.phoneBonus;
        if(lead.funding_round_date){const mo=(Date.now()-new Date(lead.funding_round_date))/(1000*60*60*24*30);if(mo<=18)score+=w.fundingWeight;}
        if(lead.linkedin_follower_count>1000) score+=3;
        if(lead.job_postings_count>5) score+=2;
        if(lead.twitter_url) score+=1;
        if(lead.hiring_surge) score+=w.hiringSurgeBonus;
        if(lead.aws_services?.length>=3) score+=w.awsBonus;
        return Math.min(100,Math.round(score));
}

function scoreLabel(score, icp) {
        const hot=(icp?.hotThreshold??DEFAULT_ICP.hotThreshold);
        const warm=(icp?.warmThreshold??DEFAULT_ICP.warmThreshold);
...
async function enrichPerson(p, apiKey, searchedDomain, icp) {
        try {
                  const r = await fetch('https://api.apollo.io/api/v1/people/match', {
                              method:'POST', headers:{'Content-Type':'application/json','X-Api-Key':apiKey},
                              body:JSON.stringify({ id: p.id, reveal_personal_emails: true, reveal_phone_number: true })
                  });
                  ...
                  if(recently_funded&&funding_stage) intent_signals.push({type:'funding',label:funding_stage+(funding_round_amount?(funding_round_amount>=1e9?' $'+(funding_round_amount/1e9).toFixed(1)+'B':funding_round_amount>=1e6?' $'+(funding_round_amount/1e6).toFixed(0)+'M':''):'')});
                  else if(recently_funded) intent_signals.push({type:'funding',label:'Recently Funded'});
                  ...
                  const lead = {
                              ...
                              email, email_status:ep.email_status||ep.contact_email_status||'', linkedin_url:ep.linkedin_url||'', twitter_url:ep.twitter_url||'', github_url:ep.github_url||'', photo_url:ep.photo_url||'',
                              city:ep.city||p.city||'', state:ep.state||p.state||'', country:ep.country||p.country||'',
                              prev_jobs:prevJobs, time_in_role_months:currentJob.start_date?Math.floor((Date.now()-new Date(currentJob.start_date))/(1000*60*60*24*30)):null, intent_strength:ep.intent_strength||null,
                              company_name:org.name||p.organization?.name||'', company_domain:searchedDomain||org.primary_domain||p.organization?.primary_domain||'',
                              company_industry:org.industry||p.organization?.industry||'', subindustry:org.subindustry||'',
                              company_size:org.estimated_num_employees||p.organization?.estimated_num_employees||'', company_founded:org.founded_year||'',
                              company_linkedin:org.linkedin_url||'', company_twitter:org.twitter_url||'', company_description:(org.short_description||'').slice(0,160), seo_description:org.seo_description||'',
                              personal_phone, all_phones, company_phone:org.primary_phone?.number||org.phone||null,
                              company_street:org.street_address||null, company_zip:org.postal_code||null, company_city:org.city||null, company_state:org.state||null, company_country:org.country||null,
                              market_cap:org.market_cap||null, tech_stack:techStack, aws_services, funding_stage, funding_total:org.total_funding||null,
                              funding_round_date, funding_round_type, funding_round_amount, top_investors, num_funding_rounds:org.num_funding_rounds||fundingEvents.length||null,
                              headcount_growth, linkedin_follower_count:org.linkedin_follower_count||null,
                              annual_revenue:fmtRevenue(org.annual_revenue_printed||org.annual_revenue||null), alexa_rank:org.alexa_rank||null,
                              job_postings_count, keywords:(org.keywords||[]).slice(0,5), hiring_surge:hiring_surge||false, recently_funded, intent_signals,
                  };
...
async function getOrgIdByDomain(domain, apiKey) {
        try {
                  ...
                  console.log('[org-enrich] domain='+domain+' orgId='+orgId);
                  return orgId;
        } catch(e) { console.log('[org-enrich] error for domain='+domain+': '+e.message); return null; }
}
...
export default async function handler(req, res) {
        ...
        try {
                  const { industry=[], employee_ranges=[], tech_stack=[], titles=[], locations=[], page=1, per_page=25, email, linkedin_url, changed_jobs_recently, organization_domains=[], icp_weights } = req.body;
                  const apiKey = process.env.APOLLO_API_KEY;
                  const icp = icp_weights||null;
          ...
          if(email||linkedin_url) {
                      ...
                      const all_phones = Array.isArray(phoneNums)?phoneNums.map(x=>x.sanitized_number||x.raw_number||x.number||'').filter(Boolean):(personal_phone?[personal_phone]:[]);
                      const lead = {
                                    id:p.id, name:p.name||'', first_name:p.first_name||'', last_name:p.last_name||'', title:p.title||'', seniority:p.seniority||'', department:p.departments?.[0]||'',
                                    email:p.email||'', email_status:p.email_status||'', linkedin_url:p.linkedin_url||'', twitter_url:p.twitter_url||'', github_url:p.github_url||'', photo_url:p.photo_url||'',
                                    city:p.city||'', state:p.state||'', country:p.country||'', company_name:org.name||'', company_domain:org.primary_domain||'', company_industry:org.industry||'',
                                    company_size:org.estimated_num_employees||'', company_founded:org.founded_year||'', company_linkedin:org.linkedin_url||'', company_description:(org.short_description||'').slice(0,160), seo_description:org.seo_description||'',
                                    personal_phone, all_phones, company_phone:org.primary_phone?.number||org.phone||null, tech_stack:techStack, aws_services,
                                    ...
                                    keywords:(org.keywords||[]).slice(0,5), hiring_surge:false, recently_funded:false, intent_signals:[],
                      };
                      lead.score = calcLeadScore(lead, icp);
                      lead.score_label = scoreLabel(lead.score, icp);
          ...
          const body = { api_key:apiKey, page, per_page };
                  if(isDomainSearch) {
                              const domain = organization_domains[0];
                              const orgId = await getOrgIdByDomain(domain, apiKey);
                              if(orgId) { body.organization_ids=[orgId]; } else { body.organization_domains=organization_domains; }
                              body.contact_email_status=['verified','guessed','unavailable','bounced'];
                  } else {
                              body.contact_email_status=['verified','guessed'];
                              body.organization_technology_names=['Amazon Web Services'].concat((tech_stack||[]).filter(t=>t!=='AWS'&&t!=='Amazon Web Services'));
                  }
                  ...
                  const searchResp = await fetch('https://api.apollo.io/api/v1/mixed_people/api_search', { method:'POST', headers:{'Content-Type':'application/json','Cache-Control':'no-cache','X-Api-Key':apiKey}, body:JSON.stringify(body) });
                  const searchData = await searchResp.json();
                  console.log('[apollo] total_entries=',searchData.pagination?.total_entries,'people=',searchData.people?.length,'error=',searchData.error);
                  ...
                  const candidates = isDomainSearch?(searchData.people||[]):(searchData.people||[]).filter(p=>p.has_email);
                  console.log('[apollo] candidates:',candidates.length,'isDomainSearch:',isDomainSearch);
                  if(!candidates.length) return res.status(200).json([]);
                  const enriched = await Promise.all(candidates.map(p=>enrichPerson(p,apiKey,isDomainSearch?organization_domains[0]:null,icp)));
