...
export default async function handler(req, res) {
  ...
  try {
        ...
        const ph=personal_phone||company_phone||null;
          const ct={};
          if(first_name) ct.firstname=first_name; if(last_name) ct.lastname=last_name; if(email) ct.email=email; if(title) ct.jobtitle=title; if(company_name) ct.company=company_name; if(company_domain) ct.website="https://"+company_domain; if(linkedin_url) ct.linkedinbio=linkedin_url; if(city) ct.city=city; if(state) ct.state=state; if(country) ct.country=country; if(twitter_url) ct.twitterhandle=twitter_url; if(ph) ct.phone=ph; if(defaultOwnerId) ct.hubspot_owner_id=defaultOwnerId; if(score_label) ct.lifecyclestage=score_label==="hot"?"salesqualifiedlead":score_label==="warm"?"marketingqualifiedlead":"lead";
