...
// Per-user Anthropic key from Redis; falls back to env var
// Falls back to smart template if no key or credits exhausted
...
function pickBestHook(body) {
    ...
    if (recently_funded && funding_stage) return { hook: 'Congrats on the ' + funding_stage + ' at ' + co, tier: 'funding' };
    if (aws_services && aws_services.length >= 2) return { hook: 'Noticed ' + co + ' is running ' + aws_services.slice(0, 2).join(' and ') + ' on AWS', tier: 'aws' };
    if (aws_services && aws_services.length === 1) return { hook: 'Noticed ' + co + ' is on ' + aws_services[0], tier: 'aws' };
    ...
    if (prev_jobs && prev_jobs.length > 0 && prev_jobs[0].company) return { hook: 'Noticed your background at ' + prev_jobs[0].company + ' before ' + co, tier: 'background' };
...
function buildPainPoint(hook, body) {
    ...
    switch (hook.tier) {
      case 'funding_rich':
      case 'funding':
              return isExec ? 'With fresh capital in the bank, cloud spend tends to accelerate fast. Most series-stage companies we talk to are running 25-40% more AWS than they need to — and it usually shows up as a nasty surprise on the next board deck.' : 'Post-raise is usually when AWS spend starts compounding quietly. Engineering ships fast, infra grows, and by the time anyone looks the bill is 2x what it was at close.';
      case 'aws':
              if (aws_services && aws_services.length >= 3) return 'With ' + aws_services.length + ' AWS services in the stack, spend tends to spread across a lot of line items. Most teams we talk to find 18-30% in recoverable savings from rightsizing and commitment coverage.';
...
export default async function handler(req, res) {
  ...
  // Per-user Anthropic key, fall back to env var
