...
export default function Home() {
  ...
  const handleCardClick = (featureId) => {
    if (featureId === 'salesanalytics') {
      setShowHome(false);
      setActiveTab('leads');
      setTimeout(() => {
        if (typeof window !== 'undefined' && typeof window.qtLaunch === 'function') {
          window.qtLaunch();
        }
      }, 150);
      return;
      ...
      <div className="app-layout">
        <aside className="sidebar">
          ...
          <nav className="sidebar-nav">
            ...
            <button className="sidebar-btn" style={{ color: '#818cf8' }} onClick={() => {
              if (typeof window !== 'undefined' && typeof window.qtLaunch === 'function') window.qtLaunch();
            }}>
              Sales Analytics
            </button>
            {isAdmin && (
