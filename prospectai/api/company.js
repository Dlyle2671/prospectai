export default async function handler(req, res) {
  ...
  try {
        ...
        // Extract LinkedIn company slug from Apollo's linkedin_url
        const linkedinSlug = org.linkedin_url
            ? (org.linkedin_url.match(/linkedin\.com\/company\/([^/?#]+)/) || [])[1] || ""
                  : "";
        ...
        async function tryGreenhouse(slug) {
                  try {
                              ...
                              return (d.jobs || []).filter(j => isTechRole(j.title)).slice(0, 15).map(j => ({ title: j.title || "", location: (j.location && j.location.name) || "", url: j.absolute_url || "", date: j.updated_at ? new Date(j.updated_at).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) : "", source: "Greenhouse" }));
                  } catch(_) { return []; }
        }
        ...
        async function tryLever(slug) {
                  try {
                              ...
                              if (!r.ok) return [];
                              const d = await r.json();
                              return (Array.isArray(d) ? d : []).filter(j => isTechRole(j.text)).slice(0, 15).map(j => ({ title: j.text || "", location: (j.categories && j.categories.location) || "", url: j.hostedUrl || j.applyUrl || "", date: j.createdAt ? new Date(j.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) : "", source: "Lever" }));
                  } catch(_) { return []; }
        }
        ...
        async function tryAshby(slug) {
                  try {
                              ...
                              if (!r.ok) return [];
                              const d = await r.json();
                              return (Array.isArray(d.jobPostings) ? d.jobPostings : Array.isArray(d.jobs) ? d.jobs : []).filter(j => isTechRole(j.title)).slice(0, 15).map(j => ({ title: j.title || "", location: j.locationName || j.location || "", url: j.jobUrl || j.applyUrl || "", date: j.publishedDate ? new Date(j.publishedDate).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) : "", source: "Ashby" }));
                  } catch(_) { return []; }
        }

        async function tryWorkable(slug) {
                  try {
                              const r = await fetch(`https://apply.workable.com/api/v3/accounts/${slug}/jobs`, { method: "POST", headers: { "Content-Type": "application/json", "User-Agent": "ProspectAI/1.0" }, body: JSON.stringify({ query: "", department: [], location: [], workplace: [], remote: null }) });
                              if (!r.ok) return [];
                              const d = await r.json();
                              return (Array.isArray(d.results) ? d.results : []).filter(j => isTechRole(j.title)).slice(0, 15).map(j => ({ title: j.title || "", location: j.location || j.city || "", url: `https://apply.workable.com/${slug}/j/${j.shortcode}/`, date: j.published_on ? new Date(j.published_on).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) : "", source: "Workable" }));
                  } catch(_) { return []; }
        }
        ...
        async function tryBambooHR(slug) {
                  try {
                              ...
                              if (!r.ok) return [];
                              const d = await r.json();
                              return (Array.isArray(d.result) ? d.result : []).filter(j => isTechRole(j.jobOpeningName)).slice(0, 15).map(j => ({ title: j.jobOpeningName || "", location: j.location && j.location.city ? j.location.city + (j.location.state ? ", " + j.location.state : "") : "", url: `https://${slug}.bamboohr.com/careers/${j.id}`, date: "", source: "BambooHR" }));
                  } catch(_) { return []; }
        }

        // LinkedIn guest jobs API - no auth required, searches by company name/slug
        async function tryLinkedIn(companyName, liSlug) {
                  try {
                              const searchTerm = liSlug || companyName;
                              const url = `https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=${encodeURIComponent(companyName)}&location=&f_C=&start=0&count=25`;
                    ...
                    const liMatches = html.match(/<li[^>]*class="[^"]*result-card[^"]*"[\s\S]*?<\/li>/g) || [];
                              // Fallback: match any <li> blocks containing job data
                    const blocks = liMatches.length > 0 ? liMatches : (html.match(/<li>[\s\S]*?<\/li>/g) || []);
                              for (const block of blocks) {
                                            const titleMatch = block.match(/class="[^"]*result-card__title[^"]*"[^>]*>([\s\S]*?)<\/h3>/i)
                                              || block.match(/<h3[^>]*>([\s\S]*?)<\/h3>/i);
                                            const title = titleMatch ? titleMatch[1].replace(/<[^>]+>/g, "").trim() : "";
                                            if (!title) continue;

                                // Only include jobs from the target company
                                const companyMatch = block.match(/class="[^"]*subtitle[^"]*"[^>]*>([\s\S]*?)<\/h4>/i)
                                              || block.match(/<h4[^>]*>([\s\S]*?)<\/h4>/i);
                                            const jobCompany = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, "").trim() : "";
                                            if (jobCompany && !jobCompany.toLowerCase().includes(companyName.toLowerCase().split(" ")[0])) continue;

                                const locationMatch = block.match(/class="[^"]*location[^"]*"[^>]*>([\s\S]*?)<\/span>/i)
                                              || block.match(/<span[^>]*location[^>]*>([\s\S]*?)<\/span>/i);
                                            const location = locationMatch ? locationMatch[1].replace(/<[^>]+>/g, "").trim() : "";

                                const urlMatch = block.match(/href="(https:\/\/www\.linkedin\.com\/jobs\/view\/[^"?]+)/i);
                                            const jobUrl = urlMatch ? urlMatch[1] : "";

                                const dateMatch = block.match(/class="[^"]*date[^"]*"[^>]*>([\s\S]*?)<\/time>/i)
                                              || block.match(/<time[^>]*>([\s\S]*?)<\/time>/i);
                                            const date = dateMatch ? dateMatch[1].replace(/<[^>]+>/g, "").trim() : "";

                                jobs.push({ title, location, url: jobUrl, date, source: "LinkedIn" });
                              }
                              return jobs.slice(0, 15);
                  } catch(_) { return []; }
        }

        const [res1, res2, jobsResp, newsResp, linkedInJobs, ...externalJobResults] = await Promise.all([
                  fetch("https://api.apollo.io/api/v1/mixed_people/api_search", {
                              method: "POST",
                              headers: { "Content-Type": "application/json", "X-Api-Key": apiKey },
                              body: JSON.stringify({ api_key: apiKey, ...(orgId ? { organization_ids: [orgId] } : { organization_domains: [cleanDomain] }), person_seniorities: ["c_suite", "founder", "vp", "director", "head", "manager", "senior"], contact_email_status: ["verified", "guessed"], per_page: 50, page: 1 }),
                  }),
                  fetch("https://api.apollo.io/api/v1/mixed_people/api_search", {
                              method: "POST",
                              headers: { "Content-Type": "application/json", "X-Api-Key": apiKey },
                              body: JSON.stringify({ api_key: apiKey, organization_domains: [cleanDomain], person_seniorities: ["c_suite", "founder", "vp", "director", "head", "manager", "senior"], contact_email_status: ["verified", "guessed"], per_page: 50, page: 1 }),
                  }),
                  ...
                  tryLinkedIn(org.name || cleanDomain, linkedinSlug),
        ...
        let apolloJobs = [];
          if (jobsResp && jobsResp.ok) {
                    try {
                                ...
                                }).slice(0, 15).map(j => ({ title: j.title || j.job_title || "", location: [j.city, j.state, j.country].filter(Boolean).join(", ") || j.location || "", url: j.url || j.job_url || j.linkedin_url || "", date: j.posted_at ? new Date(j.posted_at).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) : "", source: "Apollo" }));
                    } catch(_) {}
          }
        ...
        const allExternal = externalJobResults.flat();
          ...
          const deduped = [];
          // LinkedIn jobs go first, then Apollo, then other boards
        for (const j of [...linkedInJobs, ...apolloJobs, ...allExternal]) {
        ...
        let newsArticles = [];
          if (newsResp && newsResp.ok) {
                    ...
                    newsArticles = items.slice(0, 8).map(item => {
                                ...
                                var orgN = org.name || cleanDomain;
                                var urlOk = !!(a.url && a.url.toLowerCase().indexOf(cleanDomain) !== -1);
                                if (urlOk) return true;
                                var t4 = a.title;
                                var startsOk = t4.indexOf(orgN) === 0;
                                var verbRe = new RegExp("\\b" + orgN + "\\s+(and|or|announces|launches|raises|acquires|partners|releases|welcomes|named|wins|joins|hires|secures|is |has )", "i");
                                var verbOk = verbRe.test(t4);
                                if (orgN.length <= 9) return startsOk || verbOk;
                                return new RegExp("\\b" + orgN + "\\b", "i").test(t4);
                    });
          }
        ...
        const contacts = await Promise.all(
                  candidates.slice(0, 30).map(async (p) => {
                              try {
                                            ...
                                            return { id: ep.id || p.id, name: ep.name || p.name || "", first_name: ep.first_name || p.first_name || "", last_name: ep.last_name || p.last_name || "", title: ep.title || p.title || "", seniority: ep.seniority || p.seniority || "", email, email_status: ep.email_status || "", linkedin_url: ep.linkedin_url || "", twitter_url: ep.twitter_url || "", photo_url: ep.photo_url || "", city: ep.city || p.city || "", state: ep.state || p.state || "", country: ep.country || p.country || "", personal_phone, _hsSent: false };
                              } catch (_) { return null; }
                  })
                );
        ...
        return res.status(200).json({
                  ...
                  funding_events: sortedFunding.slice(0, 8).map(e => ({ date: e.date || e.announced_on || "", type: e.type || e.round_type || "", amount: e.amount || e.raised_amount || null, investors: (Array.isArray(e.investors) ? e.investors : Array.isArray(e.lead_investors) ? e.lead_investors : []).map(i => typeof i === "string" ? i : i.name || "").filter(Boolean).slice(0, 4) })),
