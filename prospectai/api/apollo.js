export default async function handler(req, res) {
  ...
  try {
      ...
      const people = (data.people || []).map(p => ({
              ...
              title: p.title || "",
              email: (p.email_status === "verified" || p.email_status === "guessed") ? (p.email || "") : "",
              phone: (p.phone_numbers && p.phone_numbers.length > 0) ? p.phone_numbers[0].sanitized_number || p.phone_numbers[0].raw_number || "" : "",
